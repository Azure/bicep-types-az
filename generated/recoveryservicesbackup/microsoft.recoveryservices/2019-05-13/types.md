...
### Properties
* **enhancedSecurityState**: 'Disabled' | 'Enabled' | 'Invalid' (ReadOnly): Enabled or Disabled.
* **softDeleteFeatureState**: 'Disabled' | 'Enabled' | 'Invalid' (ReadOnly): Soft Delete feature state
* **storageModelType**: 'GeoRedundant' | 'Invalid' | 'LocallyRedundant' (ReadOnly): Storage type.
* **storageType**: 'GeoRedundant' | 'Invalid' | 'LocallyRedundant' (ReadOnly): Storage type.
* **storageTypeState**: 'Invalid' | 'Locked' | 'Unlocked' (ReadOnly): Locked or Unlocked. Once a machine is registered against a resource, the storageTypeState is always Locked.
...
* **healthStatus**: 'ActionRequired' | 'ActionSuggested' | 'Invalid' | 'Passed': backups running status for this backup item.
