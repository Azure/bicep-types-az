# Microsoft.GuestConfiguration @ 2024-04-05

## Resource Microsoft.GuestConfiguration/guestConfigurationAssignments@2024-04-05
* **Valid Scope(s)**: Extension
### Properties
* **apiVersion**: '2024-04-05' (ReadOnly, DeployTimeConstant): The resource api version
...
### Properties
* **actionAfterReboot**: 'ContinueConfiguration' | 'StopConfiguration' | string (ReadOnly): Specifies what happens after a reboot during the application of a configuration. The possible values are ContinueConfiguration and StopConfiguration
* **allowModuleOverwrite**: bool (ReadOnly): If true - new configurations downloaded from the pull service are allowed to overwrite the old ones on the target node. Otherwise, false
* **configurationMode**: 'ApplyAndAutoCorrect' | 'ApplyAndMonitor' | 'ApplyOnly' | string (ReadOnly): Specifies how the LCM(Local Configuration Manager) actually applies the configuration to the target nodes. Possible values are ApplyOnly, ApplyAndMonitor, and ApplyAndAutoCorrect.
...
* **rebootIfNeeded**: bool (ReadOnly): Set this to true to automatically reboot the node after a configuration that requires reboot is applied. Otherwise, you will have to manually reboot the node for any configuration that requires it. The default value is false. To use this setting when a reboot condition is enacted by something other than DSC (such as Windows Installer), combine this setting with the xPendingReboot module.
...
### Properties
* **assignmentSource**: string (ReadOnly): Specifies the origin of the configuration.
...
* **contentManagedIdentity**: string: Managed identity with storage access of the guest configuration package and configuration parameters.
