...
### Custom
#### Properties
* **kind**: 'Custom' (Required): Network admin rule.
* **properties**: [UserRulePropertiesFormat](#userrulepropertiesformat): Security rule resource.

### Default
#### Properties
* **kind**: 'Default' (Required): Network default admin rule.
...
* **isGlobal**: 'False' | 'True': Flag if global is supported.
...
* **provisioningState**: 'Deleting' | 'Failed' | 'Succeeded' | 'Updating' (ReadOnly): The provisioning state of the security configuration user rule resource.
...
* **direction**: 'Inbound' | 'Outbound' (ReadOnly): Indicates if the traffic matched against the rule in inbound or outbound.
