# Microsoft.AzureStack
  
> [!NOTE]
> The code samples in this document are generated from API usage examples contributed by Resource Providers in their [Azure Rest API specifications](https://github.com/Azure/azure-rest-api-specs). Any issues should be reported and addressed in the source.


## microsoft.azurestack/registrations

Create or update an Azure Stack registration.
```bicep
resource exampleResource 'Microsoft.AzureStack/registrations@2017-06-01' = {
  name: 'example'
  location: 'global'
  properties: {
    registrationToken: ''
  }
}
```

## microsoft.azurestack/registrations/customersubscriptions

Creates a new customer subscription under a registration.
```bicep
resource exampleResource 'Microsoft.AzureStack/registrations/customerSubscriptions@2017-06-01' = {
  parent: parentResource 
  name: 'example'
  properties: {
    tenantId: 'dbab3982-796f-4d03-9908-044c08aef8a2'
  }
}
```
