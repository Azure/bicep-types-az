...
* **scope**: 'Pool' | 'Task': The default value is Pool. If the pool is running Windows a value of Task should be specified if stricter isolation between tasks is required. For example, if the task mutates the registry in a way which could impact other tasks.
...
### Properties
* **identityReference**: [PoolIdentityReference](#poolidentityreference): The reference of one of the pool identities to encrypt Disk. This identity will be used to access the KeyVault.
...
### Properties
* **id**: string: The ARM resource id of the disk encryption set. The resource should be in the same subscription as the Batch account.
...
### Properties
* **resourceId**: string: The ARM resource id of the user assigned identity. This reference must be included in the pool identities.
...
* **imds**: [HostEndpointSettings](#hostendpointsettings): Specifies particular host endpoint settings.
* **wireServer**: [HostEndpointSettings](#hostendpointsettings): Specifies particular host endpoint settings.
...
### Properties
* **jobDefaultOrder**: 'CreationTime' | 'None' | string: If not specified, the default is none.
