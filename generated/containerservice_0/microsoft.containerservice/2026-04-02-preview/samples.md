# Microsoft.ContainerService
  
> [!NOTE]
> The code samples in this document are generated from API usage examples contributed by Resource Providers in their [Azure Rest API specifications](https://github.com/Azure/azure-rest-api-specs). Any issues should be reported and addressed in the source.


## microsoft.containerservice/aimanagers

Creates or updates an AI Manager resource.
```bicep
resource exampleResource 'Microsoft.ContainerService/aiManagers@2026-04-02-preview' = {
  name: 'example'
  identity: {
    type: 'SystemAssigned'
  }
  location: 'eastus'
  properties: {
    deletePolicy: 'Keep'
  }
  tags: {
    key1: 'value1'
  }
}
```

## microsoft.containerservice/aimanagers/namespaces

Creates or updates an AI Manager namespace resource.
```bicep
resource exampleResource 'Microsoft.ContainerService/aiManagers/namespaces@2026-04-02-preview' = {
  parent: parentResource 
  name: 'example'
  properties: {
    annotations: {
      note: 'example'
    }
    labels: {
      app: 'myapp'
    }
  }
}
```
