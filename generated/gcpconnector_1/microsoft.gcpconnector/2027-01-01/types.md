...
## Resource Microsoft.GcpConnector/cloudFunctions@2027-01-01
...
* **properties**: [FunctionProperties](#functionproperties): The resource-specific properties for this resource.
* **systemData**: [SystemData](#systemdata) (ReadOnly): Azure Resource Manager metadata containing createdBy and modifiedBy information.
* **tags**: [TrackedResourceTags](#trackedresourcetags): Resource tags.
* **type**: 'Microsoft.GcpConnector/cloudFunctions' (ReadOnly, DeployTimeConstant): The resource type

## BuildConfig
### Properties
* **automaticUpdatePolicy**: any: AutomaticUpdatePolicy of BuildConfig.
* **build**: string: Output only. The Cloud Build name of the latest successful deployment of the function.
* **dockerRegistry**: 'ARTIFACT_REGISTRY' | 'CONTAINER_REGISTRY' | 'DOCKER_REGISTRY_UNSPECIFIED' | string: Docker Registry to use for this deployment. This configuration is only applicable to 1st Gen functions, 2nd Gen functions can only use Artifact Registry. If `docker_repository` field is specified, this field will be automatically set as `ARTIFACT_REGISTRY`. If unspecified, it currently defaults to `CONTAINER_REGISTRY`. This field may be overridden by the backend for eligible deployments.
...
* **environmentVariables**: [BuildConfigEnvironmentVariables](#buildconfigenvironmentvariables): User-provided build-time environment variables for the function
* **onDeployUpdatePolicy**: [OnDeployUpdatePolicy](#ondeployupdatepolicy): OnDeployUpdatePolicy of BuildConfig.
...
* **source**: [Source](#source): The location of the function source code.
* **sourceProvenance**: [SourceProvenance](#sourceprovenance): Output only. A permanent fixed identifier for source.
...
## BuildConfigEnvironmentVariables
### Properties
...
* **eventFilters**: [EventFilter](#eventfilter)[]: Criteria used to filter events.
...
* **retryPolicy**: 'RETRY_POLICY_DO_NOT_RETRY' | 'RETRY_POLICY_RETRY' | 'RETRY_POLICY_UNSPECIFIED' | string: Optional. If unset, then defaults to ignoring failures (i.e. not retrying them).
...
* **gcpProperties**: [GcpFunctionProperties](#gcpfunctionproperties): GCP Properties
* **gcpRegion**: string: GCP Region
* **gcpSourceSchema**: string: GCP Source Schema
* **gcpTags**: [FunctionPropertiesGcpTags](#functionpropertiesgcptags): GCP Tags
...
## FunctionPropertiesGcpTags
### Properties
### Additional Properties
* **Additional Properties Type**: string

## GcpFunctionProperties
### Properties
* **buildConfig**: [BuildConfig](#buildconfig): Describes the Build step of the function that builds a container from the given source.
...
* **environment**: 'ENVIRONMENT_UNSPECIFIED' | 'GEN_1' | 'GEN_2' | string: Describe whether the function is 1st Gen or 2nd Gen.
* **eventTrigger**: [EventTrigger](#eventtrigger): An Eventarc trigger managed by Google Cloud Functions that fires events in response to a condition in another service.
...
* **labels**: [GcpFunctionPropertiesLabels](#gcpfunctionpropertieslabels): Labels associated with this Cloud Function.
...
* **serviceConfig**: [ServiceConfig](#serviceconfig): Describes the Service being deployed. Currently deploys services to Cloud Run (fully managed).
* **state**: 'ACTIVE' | 'DELETING' | 'DEPLOYING' | 'DETACHING' | 'DETACH_FAILED' | 'FAILED' | 'STATE_UNSPECIFIED' | 'UNKNOWN' | string: Output only. State of the function.
* **stateMessages**: [GoogleCloudFunctionsV2StateMessage](#googlecloudfunctionsv2statemessage)[]: Output only. State Messages for this Cloud Function.
...
* **severity**: 'ERROR' | 'INFO' | 'SEVERITY_UNSPECIFIED' | 'WARNING' | string: Severity of the state message.
...
* **projectId**: string: Project identifier (preferably project number but can also be the project ID) of the project that contains the secret. If not set, it is assumed that the secret is in the same project as the function.
* **secret**: string: Name of the secret in secret manager (not the full resource name).
* **versions**: [SecretVersion](#secretversion)[]: List of secret versions to mount for this secret. If empty, the `latest` version of the secret will be made available in a file named after the secret under the mount point.

## ServiceConfig
### Properties
* **allTrafficOnLatestRevision**: bool: Whether 100% of traffic is routed to the latest revision. On CreateFunction and UpdateFunction, when set to true, the revision being deployed will serve 100% of traffic, ignoring any traffic split settings, if any. On GetFunction, true will be returned if the latest revision is serving 100% of traffic.
...
* **environmentVariables**: [ServiceConfigEnvironmentVariables](#serviceconfigenvironmentvariables): Environment variables that shall be available during function execution.
* **ingressSettings**: 'ALLOW_ALL' | 'ALLOW_INTERNAL_AND_GCLB' | 'ALLOW_INTERNAL_ONLY' | 'INGRESS_SETTINGS_UNSPECIFIED' | string: The ingress settings for the function, controlling what traffic can reach it.
* **maxInstanceCount**: int: The limit on the maximum number of function instances that may coexist at a given time. In some cases, such as rapid traffic surges, Cloud Functions may, for a short period of time, create more instances than the specified max instances limit. If your function cannot tolerate this temporary behavior, you may want to factor in a safety margin and set a lower max instances value than your function can tolerate. See the [Max Instances](https://cloud.google.com/functions/docs/max-instances) Guide for more details.
* **maxInstanceRequestConcurrency**: int: Sets the maximum number of concurrent requests that each instance can receive. Defaults to 1.
...
* **secretEnvironmentVariables**: [SecretEnvVar](#secretenvvar)[]: Secret environment variables configuration.
* **secretVolumes**: [SecretVolume](#secretvolume)[]: Secret volumes configuration.
* **securityLevel**: 'SECURE_ALWAYS' | 'SECURE_OPTIONAL' | 'SECURITY_LEVEL_UNSPECIFIED' | string: Security level configure whether the function only accepts https. This configuration is only applicable to 1st Gen functions with Http trigger. By default https is optional for 1st Gen functions; 2nd Gen functions are https ONLY.
...
* **timeoutSeconds**: int: The function execution timeout. Execution is considered failed and can be terminated if the function is not completed at the end of the timeout period. Defaults to 60 seconds.
...
* **vpcConnectorEgressSettings**: 'ALL_TRAFFIC' | 'PRIVATE_RANGES_ONLY' | 'VPC_CONNECTOR_EGRESS_SETTINGS_UNSPECIFIED' | string: The egress settings for the connector, controlling what traffic is diverted through it.

## ServiceConfigEnvironmentVariables
### Properties
### Additional Properties
* **Additional Properties Type**: string
...
* **repoSource**: [RepoSource](#reposource): If provided, get the source from this location in a Cloud Source Repository.
* **storageSource**: [StorageSource](#storagesource): If provided, get the source from this location in Google Cloud Storage.

## SourceProvenance
### Properties
* **gitUri**: string: A copy of the build's `source.git_uri`, if exists, with any commits resolved.
* **resolvedRepoSource**: [RepoSource](#reposource): A copy of the build's `source.repo_source`, if exists, with any revisions resolved.
* **resolvedStorageSource**: [StorageSource](#storagesource): A copy of the build's `source.storage_source`, if exists, with any generations resolved.
