# Microsoft.RecoveryServices

## microsoft.recoveryservices/vaults/certificates

Download vault credential file
```bicep
resource exampleResource 'Microsoft.RecoveryServices/vaults/certificates@2016-06-01' = {
  parent: parentResource 
  name: 'example'
  properties: {
    authType: 'AAD'
    certificate: ''
  }
}
```

## microsoft.recoveryservices/vaults

Create of Update Recovery Services vault
```bicep
resource exampleResource 'Microsoft.RecoveryServices/vaults@2016-06-01' = {
  name: 'example'
  identity: {
    type: 'SystemAssigned'
  }
  location: 'West US'
  properties: {
  }
  sku: {
    name: 'Standard'
  }
}
```
