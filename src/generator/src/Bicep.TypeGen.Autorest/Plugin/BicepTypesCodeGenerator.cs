        ...
        public override async Task Generate(CodeModel serviceClient)
        {
            ...
            { 
                ...
                foreach (var result in results)
                {
                    ...
                    await Write(typesJson, Path.Combine(result.ProviderNamespace.ToLowerInvariant(), result.ApiVersion.ToLowerInvariant(), "types.json"), false);

                    var typesMarkdown = new TypeMarkdownWriter(generatedTypes).Write(result.ProviderNamespace, result.ApiVersion);
                    await Write(typesMarkdown, Path.Combine(result.ProviderNamespace.ToLowerInvariant(), result.ApiVersion.ToLowerInvariant(), "types.md"), false);
                }
            }
        }
    }
